...
integration.  This register provides the number of 16-sample sub-integrations
in the most recently completed integration.  It can be used to properly average
the data.  To determine the number of individual samples integrated, multiply
the value of this register by 16.
...
\bramn{snap/in0123}
\brams{snap/in4567} These shared BRAMs hold the snap shot data.  Each BRAM
is 2048 words deep and stores data for the four inputs implied by its name.
Within a BRAM, each 32~bit word stores four samples for all four inputs.  The
\MSB holds four samples of the BRAM's lowest numbered input; the \LSB holds
four samples of the BRAM's highest numbered input.  Within a byte, the two
\MSbs represent the first (in time) of the byte's four samples.  Thus, each
shared BRAM can hold 8192 samples for four inputs.
