...
\begin{bytefield}{32}
  \bitheader[b]{0,3,4,7,8,11,12,15,16,31} \\
  \bitbox{16}{\color[gray]{0.75}\rule{\width}{\height}} &
  \bitbox{4}{\scriptsize insel3} &
  \bitbox{4}{\scriptsize insel2} &
  \bitbox{4}{\scriptsize insel1} &
  \bitbox{4}{\scriptsize insel0}
\end{bytefield}

\begin{description}
\item[bit = 0] Select ADC for one demux path of one input
\item[bit = 1] Select noise source for one demux path of one input
\end{description}
...
millidegrees per step).  The value written into these registers can be computed
as
...
edge sensitive, so they must be cleared before to take the associated circuitry
out of reset.
