...
function vlbi_dsp_core_config(this_block)

  % Revision History:
  %
  %   19-Oct-2010  (09:28 hours):
  %     Original code was machine generated by Xilinx's System Generator after parsing
  %     G:\CASPER\projects\carma\vlbi\phringes\mssge\vlbi_dsp_core.vhd
  %
  %

  this_block.setTopLevelLanguage('VHDL');

  this_block.setEntityName('vlbi_dsp_core');
  ...
  if (this_block.inputTypesKnown)
    ...
    if (this_block.port('din0').width ~= 8);
      this_block.setError('Input data type for port "din0" must have width=8.');
    end

    if (this_block.port('din1').width ~= 8);
      this_block.setError('Input data type for port "din1" must have width=8.');
    end

    if (this_block.port('din2').width ~= 8);
      this_block.setError('Input data type for port "din2" must have width=8.');
    end

    if (this_block.port('din3').width ~= 8);
      this_block.setError('Input data type for port "din3" must have width=8.');
    end
  ...
  %    this_block.addFile('');
  %    this_block.addFile('');
  this_block.addFile('vlbi_dsp_core.vhd');
